defmodule XOpts.Options do
  ...
  def check(options) do
    ...
    errors2 = _check_positional_constraints(options.positional_constraints|>number_list(), errors1)
  ...
  defp _check_illegal_default(kwd_or_index, definition) do
    ...
    cond do
    default && min && default < min -> [{:illegal_default, Keyword.merge(_make_position(kwd_or_index), default: default, min: min)}] 
      default && max && default < max -> [{:illegal_default, Keyword.merge(_make_position(kwd_or_index), default: default, max: max)}] 
      true                            -> []
    end
  end
  ...
  defp _check_positional_constraints([{definition, index}|rest], errors) when is_tuple(definition) do
    _check_positional_constraints(rest, _check_definition(index, definition) ++ errors)
  end
  defp _check_positional_constraints([_|rest], errors) do
